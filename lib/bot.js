...
import { handler as bookOfVicHandler } from './events/book-of-vic.js';
...
bot.on(Events.InteractionCreate, async (interaction) => {
  ...
  try {
    logger.info({ msg: "Executing command", command: commandName, subcommand: interaction.options.getSubcommand(), caller: interaction.user.tag });
  } catch (error) {
    logger.info({ msg: "Executing command", command: commandName, caller: interaction.user.tag });
  }
...
bot.on(Events.MessageCreate, async (message) => {
  ...
  if (!message.author.bot && /book of vic/gi.test(message.cleanContent)) {
    await bookOfVicHandler(bot, message);
    return;
  }
});
